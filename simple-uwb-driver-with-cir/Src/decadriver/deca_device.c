...
uint8_t temp[32];
void dwt_readaccdata_without_enableclocks(uint8 *buffer, uint16 len, uint16 accOffset)
{
    dwt_readfromdevice(ACC_MEM_ID,accOffset,len,buffer);
}

void dwt_enable_acc_clocks(void)
{
    _dwt_enableclocks(READ_ACC_ON);
}

void dwt_disable_acc_clocks(void)
{
    _dwt_enableclocks(READ_ACC_OFF); // Revert clocks back
}
...
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics)
{
   ...
   diagnostics->rxPower = estRxPwr;

   //计算第一路径功率
   float estFpPwr = 10.0*log10(((float)diagnostics->firstPathAmp1*(float)diagnostics->firstPathAmp1+(float)diagnostics->firstPathAmp2*(float)diagnostics->firstPathAmp2+(float)diagnostics->firstPathAmp3*(float)diagnostics->firstPathAmp3)/((float)N*(float)N))-A;
	if(estFpPwr > -88) 
   {
		estFpPwr += (estFpPwr+88)*corrFac;
	} 
   diagnostics->fpPower = estFpPwr;

   //计算通信质量
   diagnostics->quality = (float)diagnostics->firstPathAmp2/(float)diagnostics->stdNoise;
